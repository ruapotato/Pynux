...
from lib.pid import pid_simple_init, pid_simple_set_setpoint, pid_simple_update, pid_simple_reset
from lib.pid import pid_simple_set_limits, pid_simple_get_error, pid_simple_set_gains

def test_pid_simple_init():
    print_section("PID Initialization")

    # Gains scaled by 100: Kp=100, Ki=10, Kd=5
    pid_simple_init(100, 10, 5)
    pid_simple_set_setpoint(1000)
...
def test_pid_integral():
    ...
    pid_simple_init(0, 100, 0)  # Ki = 1.0
    pid_simple_set_setpoint(1000)
    pid_simple_set_limits(-10000, 10000)

    # First update with error=500
    output1: int32 = pid_simple_update(500)

    # Second update - integral should accumulate
    output2: int32 = pid_simple_update(500)
    assert_gt(output2, output1, "integral accumulates")

    # Third update
    output3: int32 = pid_simple_update(500)
    assert_gt(output3, output2, "integral continues to grow")

def test_pid_simple_reset():
    print_section("PID Reset")

    pid_simple_init(100, 100, 0)
    pid_simple_set_setpoint(1000)
    pid_simple_set_limits(-10000, 10000)

    # Build up some integral
    pid_simple_update(500)
    pid_simple_update(500)
    pid_simple_update(500)
...
def test_pid_error():
    ...
    pid_simple_init(100, 0, 0)
    pid_simple_set_setpoint(1000)
    pid_simple_set_limits(-10000, 10000)

    pid_simple_update(600)  # Error = 1000 - 600 = 400
    error: int32 = pid_simple_get_error()
    assert_eq(error, 400, "error is setpoint - pv")

    pid_simple_update(1200)  # Error = 1000 - 1200 = -200
    error = pid_simple_get_error()
...
def test_pid_setpoint_change():
    ...
    output1: int32 = pid_simple_update(500)  # Error = 500

    pid_simple_set_setpoint(500)  # Change setpoint to match PV
    output2: int32 = pid_simple_update(500)  # Error = 0
...
def test_pid_convergence():
    ...
    pid_simple_init(50, 10, 5)  # Kp=0.5, Ki=0.1, Kd=0.05
    pid_simple_set_setpoint(1000)
    pid_simple_set_limits(0, 100)
    ...
    i: int32 = 0
    while i < 10:
        output: int32 = pid_simple_update(pv)
...
def run_pid_tests():
    ...
    test_pid_simple_init()
    test_pid_proportional()
    test_pid_limits()
    test_pid_integral()
    test_pid_simple_reset()
    test_pid_error()
