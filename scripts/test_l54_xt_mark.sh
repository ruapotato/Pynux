#!/usr/bin/env bash
# scripts/test_l54_xt_mark.sh — L54 xt_mark.ko load test.
#
# Goal:
#   Ship xt_mark.ko (netfilter mark match + MARK target). Init resolves the L54 shims and
#   returns 0.
...
STAGED_KO="$LKM_DIR/xt_mark.ko"

KREL="$(uname -r)"
HOST_LIB="/lib/modules/${KREL}/kernel"
CANDIDATES=(
    "${HOST_LIB}/net/netfilter/xt_mark.ko"
    "${HOST_LIB}/net/netfilter/xt_mark.ko.xz"
)
...
if [ -z "$picked" ]; then
    echo "L54: xt_mark.ko not present; skipping"
    exit 0
fi

echo "[test_l54_xt_mark] picked: $picked"
...
echo "[test_l54_xt_mark] UND ($(echo "$UND_SYMS" | wc -w)):"
for s in $UND_SYMS; do echo "  $s"; done
echo "[test_l54_xt_mark] MISSING:"
...
(
    sleep 3
    printf 'insmod /lib/modules/6.12/xt_mark.ko\n'
    sleep 5
    printf 'exit\n'
    sleep 1
...
if grep -E -q "PANIC|panic:" "$LOG"; then
    echo "[test_l54_xt_mark] FAIL: kernel panic"
    exit 1
fi
...
echo "[test_l54_xt_mark] init_OK=$INIT_OK lib_only=$LIB_ONLY fail=$INSMOD_FAIL"

if [ "$INSMOD_FAIL" -ge 1 ]; then echo "[test_l54_xt_mark] FAIL"; exit 1; fi
if [ "$INIT_OK" -ge 1 ] || [ "$LIB_ONLY" -ge 1 ]; then
    echo "[test_l54_xt_mark] PASS: xt_mark.ko loaded"
    exit 0
fi
echo "[test_l54_xt_mark] FAIL: no PASS markers"
exit 1
