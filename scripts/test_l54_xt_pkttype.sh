#!/usr/bin/env bash
# scripts/test_l54_xt_pkttype.sh — L54 xt_pkttype.ko load test.
#
# Goal:
#   Ship xt_pkttype.ko (netfilter packet-type match). Init resolves the L54 shims and
#   returns 0.
...
STAGED_KO="$LKM_DIR/xt_pkttype.ko"

KREL="$(uname -r)"
HOST_LIB="/lib/modules/${KREL}/kernel"
CANDIDATES=(
    "${HOST_LIB}/net/netfilter/xt_pkttype.ko"
    "${HOST_LIB}/net/netfilter/xt_pkttype.ko.xz"
)
...
if [ -z "$picked" ]; then
    echo "L54: xt_pkttype.ko not present; skipping"
    exit 0
fi

echo "[test_l54_xt_pkttype] picked: $picked"
...
echo "[test_l54_xt_pkttype] UND ($(echo "$UND_SYMS" | wc -w)):"
for s in $UND_SYMS; do echo "  $s"; done
echo "[test_l54_xt_pkttype] MISSING:"
...
(
    sleep 3
    printf 'insmod /lib/modules/6.12/xt_pkttype.ko\n'
    sleep 5
    printf 'exit\n'
    sleep 1
...
if grep -E -q "PANIC|panic:" "$LOG"; then
    echo "[test_l54_xt_pkttype] FAIL: kernel panic"
    exit 1
fi
...
echo "[test_l54_xt_pkttype] init_OK=$INIT_OK lib_only=$LIB_ONLY fail=$INSMOD_FAIL"

if [ "$INSMOD_FAIL" -ge 1 ]; then echo "[test_l54_xt_pkttype] FAIL"; exit 1; fi
if [ "$INIT_OK" -ge 1 ] || [ "$LIB_ONLY" -ge 1 ]; then
    echo "[test_l54_xt_pkttype] PASS: xt_pkttype.ko loaded"
    exit 0
fi
echo "[test_l54_xt_pkttype] FAIL: no PASS markers"
exit 1
