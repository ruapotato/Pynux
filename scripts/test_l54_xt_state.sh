#!/usr/bin/env bash
# scripts/test_l54_xt_state.sh — L54 xt_state.ko load test.
#
# Goal:
#   Ship xt_state.ko (netfilter conntrack-state match). Init resolves the L54 shims and
#   returns 0.
...
STAGED_KO="$LKM_DIR/xt_state.ko"

KREL="$(uname -r)"
HOST_LIB="/lib/modules/${KREL}/kernel"
CANDIDATES=(
    "${HOST_LIB}/net/netfilter/xt_state.ko"
    "${HOST_LIB}/net/netfilter/xt_state.ko.xz"
)
...
if [ -z "$picked" ]; then
    echo "L54: xt_state.ko not present; skipping"
    exit 0
fi

echo "[test_l54_xt_state] picked: $picked"
...
echo "[test_l54_xt_state] UND ($(echo "$UND_SYMS" | wc -w)):"
for s in $UND_SYMS; do echo "  $s"; done
echo "[test_l54_xt_state] MISSING:"
...
(
    sleep 3
    printf 'insmod /lib/modules/6.12/xt_state.ko\n'
    sleep 5
    printf 'exit\n'
    sleep 1
...
if grep -E -q "PANIC|panic:" "$LOG"; then
    echo "[test_l54_xt_state] FAIL: kernel panic"
    exit 1
fi
...
echo "[test_l54_xt_state] init_OK=$INIT_OK lib_only=$LIB_ONLY fail=$INSMOD_FAIL"

if [ "$INSMOD_FAIL" -ge 1 ]; then echo "[test_l54_xt_state] FAIL"; exit 1; fi
if [ "$INIT_OK" -ge 1 ] || [ "$LIB_ONLY" -ge 1 ]; then
    echo "[test_l54_xt_state] PASS: xt_state.ko loaded"
    exit 0
fi
echo "[test_l54_xt_state] FAIL: no PASS markers"
exit 1
