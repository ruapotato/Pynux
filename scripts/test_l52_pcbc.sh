#!/usr/bin/env bash
# scripts/test_l52_pcbc.sh — L52 pcbc.ko load test.
#
# Goal:
#   Ship pcbc.ko (Propagating CBC chaining-mode template; one crypto_template registered). 2 new exports: crypto_cipher_decrypt_one + skcipher_alloc_instance_simple.
...
STAGED_KO="$LKM_DIR/pcbc.ko"

KREL="$(uname -r)"
HOST_LIB="/lib/modules/${KREL}/kernel"
CANDIDATES=(
    "${HOST_LIB}/crypto/pcbc.ko"
    "${HOST_LIB}/crypto/pcbc.ko.xz"
)
...
if [ -z "$picked" ]; then
    echo "L52: pcbc.ko not present; skipping"
    exit 0
fi

echo "[test_l52_pcbc] picked: $picked"
...
echo "[test_l52_pcbc] UND ($(echo "$UND_SYMS" | wc -w)):"
for s in $UND_SYMS; do echo "  $s"; done
echo "[test_l52_pcbc] MISSING:"
...
(
    sleep 3
    printf 'insmod /lib/modules/6.12/pcbc.ko\n'
    sleep 5
    printf 'exit\n'
    sleep 1
...
if grep -E -q "PANIC|panic:" "$LOG"; then
    echo "[test_l52_pcbc] FAIL: kernel panic"
    exit 1
fi
...
echo "[test_l52_pcbc] init_OK=$INIT_OK lib_only=$LIB_ONLY fail=$INSMOD_FAIL"

if [ "$INSMOD_FAIL" -ge 1 ]; then echo "[test_l52_pcbc] FAIL"; exit 1; fi
if [ "$INIT_OK" -ge 1 ] || [ "$LIB_ONLY" -ge 1 ]; then
    echo "[test_l52_pcbc] PASS: pcbc.ko loaded"
    exit 0
fi
echo "[test_l52_pcbc] FAIL: no PASS markers"
exit 1
