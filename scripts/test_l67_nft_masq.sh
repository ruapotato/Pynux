#!/usr/bin/env bash
# scripts/test_l67_nft_masq.sh — L67 nft_masq.ko load test.
#
# Goal:
#   Ship nft_masq.ko (nftables masquerade expression — SNAT to the
#   egress interface's address). 16 UND, 4 new at L67
#   (nf_nat_masquerade_inet_register_notifiers,
#   nf_nat_masquerade_inet_unregister_notifiers,
#   nf_nat_masquerade_ipv4, nf_nat_masquerade_ipv6).
#
#   57th distro .ko to load. Fourth L67 module.
...
STAGED_KO="$LKM_DIR/nft_masq.ko"

KREL="$(uname -r)"
HOST_LIB="/lib/modules/${KREL}/kernel"
CANDIDATES=(
    "${HOST_LIB}/net/netfilter/nft_masq.ko"
    "${HOST_LIB}/net/netfilter/nft_masq.ko.xz"
)
...
if [ -z "$picked" ]; then
    echo "L67: nft_masq.ko not present; skipping"
    exit 0
fi

echo "[test_l67_nft_masq] picked: $picked"
...
echo "[test_l67_nft_masq] UND ($(echo "$UND_SYMS" | wc -w)):"
for s in $UND_SYMS; do echo "  $s"; done
echo "[test_l67_nft_masq] MISSING:"
...
(
    sleep 3
    printf 'insmod /lib/modules/6.12/nft_masq.ko\n'
    sleep 5
    printf 'exit\n'
    sleep 1
...
if grep -E -q "PANIC|panic:" "$LOG"; then
    echo "[test_l67_nft_masq] FAIL: kernel panic"
    exit 1
fi
...
echo "[test_l67_nft_masq] init_OK=$INIT_OK lib_only=$LIB_ONLY fail=$INSMOD_FAIL"

if [ "$INSMOD_FAIL" -ge 1 ]; then echo "[test_l67_nft_masq] FAIL"; exit 1; fi
if [ "$INIT_OK" -ge 1 ] || [ "$LIB_ONLY" -ge 1 ]; then
    echo "[test_l67_nft_masq] PASS: nft_masq.ko loaded"
    exit 0
fi
echo "[test_l67_nft_masq] FAIL: no PASS markers"
exit 1
