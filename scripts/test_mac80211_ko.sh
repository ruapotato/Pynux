...
if grep -aF -q "unresolved external symbol" "$LOG"; then
    echo "[test_mac80211_ko] FAIL: unresolved external symbol reported"
...
if grep -aE -q "^TRAP:|^\[[0-9]+\] TRAP:|^BUG:|^\[[0-9]+\] BUG:" "$LOG"; then
    echo "[test_mac80211_ko] FAIL: TRAP/BUG reported during boot"
...
if grep -aE -q "kmod_linux: init returned -[0-9]+" "$LOG"; then
    echo "[test_mac80211_ko] FAIL: a module's init_module returned non-zero"
