#!/usr/bin/env bash
# scripts/test_l52_nhpoly1305.sh — L52 nhpoly1305.ko load test.
#
# Goal:
#   Ship nhpoly1305.ko (NH-Poly1305 universal hash used by Adiantum; one shash_alg registered via crypto_register_shash). 1 new export: poly1305_core_emit.
...
STAGED_KO="$LKM_DIR/nhpoly1305.ko"

KREL="$(uname -r)"
HOST_LIB="/lib/modules/${KREL}/kernel"
CANDIDATES=(
    "${HOST_LIB}/crypto/nhpoly1305.ko"
    "${HOST_LIB}/crypto/nhpoly1305.ko.xz"
)
...
if [ -z "$picked" ]; then
    echo "L52: nhpoly1305.ko not present; skipping"
    exit 0
fi

echo "[test_l52_nhpoly1305] picked: $picked"
...
echo "[test_l52_nhpoly1305] UND ($(echo "$UND_SYMS" | wc -w)):"
for s in $UND_SYMS; do echo "  $s"; done
echo "[test_l52_nhpoly1305] MISSING:"
...
(
    sleep 3
    printf 'insmod /lib/modules/6.12/nhpoly1305.ko\n'
    sleep 5
    printf 'exit\n'
    sleep 1
...
if grep -E -q "PANIC|panic:" "$LOG"; then
    echo "[test_l52_nhpoly1305] FAIL: kernel panic"
    exit 1
fi
...
echo "[test_l52_nhpoly1305] init_OK=$INIT_OK lib_only=$LIB_ONLY fail=$INSMOD_FAIL"

if [ "$INSMOD_FAIL" -ge 1 ]; then echo "[test_l52_nhpoly1305] FAIL"; exit 1; fi
if [ "$INIT_OK" -ge 1 ] || [ "$LIB_ONLY" -ge 1 ]; then
    echo "[test_l52_nhpoly1305] PASS: nhpoly1305.ko loaded"
    exit 0
fi
echo "[test_l52_nhpoly1305] FAIL: no PASS markers"
exit 1
