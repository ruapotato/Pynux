#!/usr/bin/env bash
# scripts/test_l52_wp512.sh — L52 wp512.ko load test.
#
# Goal:
#   Ship wp512.ko (Whirlpool 512-bit hash; three shashes registered via crypto_register_shashes). Zero new exports needed.
...
STAGED_KO="$LKM_DIR/wp512.ko"

KREL="$(uname -r)"
HOST_LIB="/lib/modules/${KREL}/kernel"
CANDIDATES=(
    "${HOST_LIB}/crypto/wp512.ko"
    "${HOST_LIB}/crypto/wp512.ko.xz"
)
...
if [ -z "$picked" ]; then
    echo "L52: wp512.ko not present; skipping"
    exit 0
fi

echo "[test_l52_wp512] picked: $picked"
...
echo "[test_l52_wp512] UND ($(echo "$UND_SYMS" | wc -w)):"
for s in $UND_SYMS; do echo "  $s"; done
echo "[test_l52_wp512] MISSING:"
...
(
    sleep 3
    printf 'insmod /lib/modules/6.12/wp512.ko\n'
    sleep 5
    printf 'exit\n'
    sleep 1
...
if grep -E -q "PANIC|panic:" "$LOG"; then
    echo "[test_l52_wp512] FAIL: kernel panic"
    exit 1
fi
...
echo "[test_l52_wp512] init_OK=$INIT_OK lib_only=$LIB_ONLY fail=$INSMOD_FAIL"

if [ "$INSMOD_FAIL" -ge 1 ]; then echo "[test_l52_wp512] FAIL"; exit 1; fi
if [ "$INIT_OK" -ge 1 ] || [ "$LIB_ONLY" -ge 1 ]; then
    echo "[test_l52_wp512] PASS: wp512.ko loaded"
    exit 0
fi
echo "[test_l52_wp512] FAIL: no PASS markers"
exit 1
