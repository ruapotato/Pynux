#!/usr/bin/env bash
# scripts/test_l54_xt_tcpudp.sh — L54 xt_tcpudp.ko load test.
#
# Goal:
#   Ship xt_tcpudp.ko (netfilter tcp/udp/udplite matches). Init resolves the L54 shims and
#   returns 0.
...
STAGED_KO="$LKM_DIR/xt_tcpudp.ko"

KREL="$(uname -r)"
HOST_LIB="/lib/modules/${KREL}/kernel"
CANDIDATES=(
    "${HOST_LIB}/net/netfilter/xt_tcpudp.ko"
    "${HOST_LIB}/net/netfilter/xt_tcpudp.ko.xz"
)
...
if [ -z "$picked" ]; then
    echo "L54: xt_tcpudp.ko not present; skipping"
    exit 0
fi

echo "[test_l54_xt_tcpudp] picked: $picked"
...
echo "[test_l54_xt_tcpudp] UND ($(echo "$UND_SYMS" | wc -w)):"
for s in $UND_SYMS; do echo "  $s"; done
echo "[test_l54_xt_tcpudp] MISSING:"
...
(
    sleep 3
    printf 'insmod /lib/modules/6.12/xt_tcpudp.ko\n'
    sleep 5
    printf 'exit\n'
    sleep 1
...
if grep -E -q "PANIC|panic:" "$LOG"; then
    echo "[test_l54_xt_tcpudp] FAIL: kernel panic"
    exit 1
fi
...
echo "[test_l54_xt_tcpudp] init_OK=$INIT_OK lib_only=$LIB_ONLY fail=$INSMOD_FAIL"

if [ "$INSMOD_FAIL" -ge 1 ]; then echo "[test_l54_xt_tcpudp] FAIL"; exit 1; fi
if [ "$INIT_OK" -ge 1 ] || [ "$LIB_ONLY" -ge 1 ]; then
    echo "[test_l54_xt_tcpudp] PASS: xt_tcpudp.ko loaded"
    exit 0
fi
echo "[test_l54_xt_tcpudp] FAIL: no PASS markers"
exit 1
