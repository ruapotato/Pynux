#!/usr/bin/env bash
# scripts/test_l54_zstd.sh — L54 zstd.ko load test.
#
# Goal:
#   Ship zstd.ko (Zstd compression alg + scomp). Init resolves the L54 shims and
#   returns 0.
...
STAGED_KO="$LKM_DIR/zstd.ko"

KREL="$(uname -r)"
HOST_LIB="/lib/modules/${KREL}/kernel"
CANDIDATES=(
    "${HOST_LIB}/crypto/zstd.ko"
    "${HOST_LIB}/crypto/zstd.ko.xz"
)
...
if [ -z "$picked" ]; then
    echo "L54: zstd.ko not present; skipping"
    exit 0
fi

echo "[test_l54_zstd] picked: $picked"
...
echo "[test_l54_zstd] UND ($(echo "$UND_SYMS" | wc -w)):"
for s in $UND_SYMS; do echo "  $s"; done
echo "[test_l54_zstd] MISSING:"
...
(
    sleep 3
    printf 'insmod /lib/modules/6.12/zstd.ko\n'
    sleep 5
    printf 'exit\n'
    sleep 1
...
if grep -E -q "PANIC|panic:" "$LOG"; then
    echo "[test_l54_zstd] FAIL: kernel panic"
    exit 1
fi
...
echo "[test_l54_zstd] init_OK=$INIT_OK lib_only=$LIB_ONLY fail=$INSMOD_FAIL"

if [ "$INSMOD_FAIL" -ge 1 ]; then echo "[test_l54_zstd] FAIL"; exit 1; fi
if [ "$INIT_OK" -ge 1 ] || [ "$LIB_ONLY" -ge 1 ]; then
    echo "[test_l54_zstd] PASS: zstd.ko loaded"
    exit 0
fi
echo "[test_l54_zstd] FAIL: no PASS markers"
exit 1
