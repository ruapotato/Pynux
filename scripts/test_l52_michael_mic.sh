#!/usr/bin/env bash
# scripts/test_l52_michael_mic.sh — L52 michael_mic.ko load test.
#
# Goal:
#   Ship michael_mic.ko (IEEE 802.11i TKIP MIC; one shash_alg registered via crypto_register_shash). Zero new exports needed.
...
STAGED_KO="$LKM_DIR/michael_mic.ko"

KREL="$(uname -r)"
HOST_LIB="/lib/modules/${KREL}/kernel"
CANDIDATES=(
    "${HOST_LIB}/crypto/michael_mic.ko"
    "${HOST_LIB}/crypto/michael_mic.ko.xz"
)
...
if [ -z "$picked" ]; then
    echo "L52: michael_mic.ko not present; skipping"
    exit 0
fi

echo "[test_l52_michael_mic] picked: $picked"
...
echo "[test_l52_michael_mic] UND ($(echo "$UND_SYMS" | wc -w)):"
for s in $UND_SYMS; do echo "  $s"; done
echo "[test_l52_michael_mic] MISSING:"
...
(
    sleep 3
    printf 'insmod /lib/modules/6.12/michael_mic.ko\n'
    sleep 5
    printf 'exit\n'
    sleep 1
...
if grep -E -q "PANIC|panic:" "$LOG"; then
    echo "[test_l52_michael_mic] FAIL: kernel panic"
    exit 1
fi
...
echo "[test_l52_michael_mic] init_OK=$INIT_OK lib_only=$LIB_ONLY fail=$INSMOD_FAIL"

if [ "$INSMOD_FAIL" -ge 1 ]; then echo "[test_l52_michael_mic] FAIL"; exit 1; fi
if [ "$INIT_OK" -ge 1 ] || [ "$LIB_ONLY" -ge 1 ]; then
    echo "[test_l52_michael_mic] PASS: michael_mic.ko loaded"
    exit 0
fi
echo "[test_l52_michael_mic] FAIL: no PASS markers"
exit 1
