#!/usr/bin/env bash
# scripts/test_l54_lz4.sh — L54 lz4.ko load test.
#
# Goal:
#   Ship lz4.ko (LZ4 compression alg + scomp). Init resolves the L54 shims and
#   returns 0.
...
STAGED_KO="$LKM_DIR/lz4.ko"

KREL="$(uname -r)"
HOST_LIB="/lib/modules/${KREL}/kernel"
CANDIDATES=(
    "${HOST_LIB}/crypto/lz4.ko"
    "${HOST_LIB}/crypto/lz4.ko.xz"
)
...
if [ -z "$picked" ]; then
    echo "L54: lz4.ko not present; skipping"
    exit 0
fi

echo "[test_l54_lz4] picked: $picked"
...
echo "[test_l54_lz4] UND ($(echo "$UND_SYMS" | wc -w)):"
for s in $UND_SYMS; do echo "  $s"; done
echo "[test_l54_lz4] MISSING:"
...
(
    sleep 3
    printf 'insmod /lib/modules/6.12/lz4.ko\n'
    sleep 5
    printf 'exit\n'
    sleep 1
...
if grep -E -q "PANIC|panic:" "$LOG"; then
    echo "[test_l54_lz4] FAIL: kernel panic"
    exit 1
fi
...
echo "[test_l54_lz4] init_OK=$INIT_OK lib_only=$LIB_ONLY fail=$INSMOD_FAIL"

if [ "$INSMOD_FAIL" -ge 1 ]; then echo "[test_l54_lz4] FAIL"; exit 1; fi
if [ "$INIT_OK" -ge 1 ] || [ "$LIB_ONLY" -ge 1 ]; then
    echo "[test_l54_lz4] PASS: lz4.ko loaded"
    exit 0
fi
echo "[test_l54_lz4] FAIL: no PASS markers"
exit 1
