#!/usr/bin/env bash
# scripts/test_pynux_user.sh - end-to-end verification that the
# `x86_64-pynux-user` compiler target produces a userspace ELF the
# kernel actually loads, runs, and gets useful output from.
#
# Pipeline (each step is independent and re-runs cheaply):
#   1. Compile user/hello.py -> build/user/hello.elf via the new target.
#   2. Regenerate fs/initramfs_blob.S with INIT_ELF pointing at hello.elf
#      so the cpio archive's /init is the Pynux-compiled binary.
#   3. Rebuild the kernel image (init/main.py -> build/pynux-vmlinux.elf).
...
python3 -m compiler.pynux compile \
    --target=x86_64-pynux-user \
    user/hello.py \
    -o "$HELLO_ELF"

echo "[test_pynux_user] (2/5) Regenerate initramfs with /init = hello.elf"
INIT_ELF="$HELLO_ELF" python3 scripts/build_initramfs.py

echo "[test_pynux_user] (3/5) Rebuild kernel image"
python3 -m compiler.pynux compile \
    --target=x86_64-bare-metal \
    init/main.py \
    -o "$ELF"

echo "[test_pynux_user] (4/5) Boot in QEMU and capture serial (10s)"
LOG=$(mktemp)
trap 'rm -f "$LOG"' EXIT
set +e
...
echo "[test_pynux_user] (raw output) ----"
cat "$LOG"
echo "[test_pynux_user] (raw output end) ----"

echo "[test_pynux_user] (5/5) Grep for banner: '$BANNER'"
if grep -F -q "$BANNER" "$LOG"; then
    echo "[test_pynux_user] PASS: hello.py banner found in serial output."
...
echo "[test_pynux_user] FAIL: banner not found in serial log."
echo "[test_pynux_user] QEMU rc=$rc"
INIT_ELF="build/user/init.elf" python3 scripts/build_initramfs.py >/dev/null
exit 1
