#!/usr/bin/env bash
# scripts/test_l52_fcrypt.sh — L52 fcrypt.ko load test.
#
# Goal:
#   Ship fcrypt.ko (Pike's NetBSD-derived block cipher; one struct crypto_alg registered, init tail-jumps to crypto_register_alg). Zero new exports needed.
...
STAGED_KO="$LKM_DIR/fcrypt.ko"

KREL="$(uname -r)"
HOST_LIB="/lib/modules/${KREL}/kernel"
CANDIDATES=(
    "${HOST_LIB}/crypto/fcrypt.ko"
    "${HOST_LIB}/crypto/fcrypt.ko.xz"
)
...
if [ -z "$picked" ]; then
    echo "L52: fcrypt.ko not present; skipping"
    exit 0
fi

echo "[test_l52_fcrypt] picked: $picked"
...
echo "[test_l52_fcrypt] UND ($(echo "$UND_SYMS" | wc -w)):"
for s in $UND_SYMS; do echo "  $s"; done
echo "[test_l52_fcrypt] MISSING:"
...
(
    sleep 3
    printf 'insmod /lib/modules/6.12/fcrypt.ko\n'
    sleep 5
    printf 'exit\n'
    sleep 1
...
if grep -E -q "PANIC|panic:" "$LOG"; then
    echo "[test_l52_fcrypt] FAIL: kernel panic"
    exit 1
fi
...
echo "[test_l52_fcrypt] init_OK=$INIT_OK lib_only=$LIB_ONLY fail=$INSMOD_FAIL"

if [ "$INSMOD_FAIL" -ge 1 ]; then echo "[test_l52_fcrypt] FAIL"; exit 1; fi
if [ "$INIT_OK" -ge 1 ] || [ "$LIB_ONLY" -ge 1 ]; then
    echo "[test_l52_fcrypt] PASS: fcrypt.ko loaded"
    exit 0
fi
echo "[test_l52_fcrypt] FAIL: no PASS markers"
exit 1
