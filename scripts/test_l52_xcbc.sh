#!/usr/bin/env bash
# scripts/test_l52_xcbc.sh — L52 xcbc.ko load test.
#
# Goal:
#   Ship xcbc.ko (XCBC-MAC template; one crypto_template registered via crypto_register_template). Zero new exports needed (full coverage from cmac/xts plumbing in L51).
...
STAGED_KO="$LKM_DIR/xcbc.ko"

KREL="$(uname -r)"
HOST_LIB="/lib/modules/${KREL}/kernel"
CANDIDATES=(
    "${HOST_LIB}/crypto/xcbc.ko"
    "${HOST_LIB}/crypto/xcbc.ko.xz"
)
...
if [ -z "$picked" ]; then
    echo "L52: xcbc.ko not present; skipping"
    exit 0
fi

echo "[test_l52_xcbc] picked: $picked"
...
echo "[test_l52_xcbc] UND ($(echo "$UND_SYMS" | wc -w)):"
for s in $UND_SYMS; do echo "  $s"; done
echo "[test_l52_xcbc] MISSING:"
...
(
    sleep 3
    printf 'insmod /lib/modules/6.12/xcbc.ko\n'
    sleep 5
    printf 'exit\n'
    sleep 1
...
if grep -E -q "PANIC|panic:" "$LOG"; then
    echo "[test_l52_xcbc] FAIL: kernel panic"
    exit 1
fi
...
echo "[test_l52_xcbc] init_OK=$INIT_OK lib_only=$LIB_ONLY fail=$INSMOD_FAIL"

if [ "$INSMOD_FAIL" -ge 1 ]; then echo "[test_l52_xcbc] FAIL"; exit 1; fi
if [ "$INIT_OK" -ge 1 ] || [ "$LIB_ONLY" -ge 1 ]; then
    echo "[test_l52_xcbc] PASS: xcbc.ko loaded"
    exit 0
fi
echo "[test_l52_xcbc] FAIL: no PASS markers"
exit 1
