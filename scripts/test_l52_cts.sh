#!/usr/bin/env bash
# scripts/test_l52_cts.sh — L52 cts.ko load test.
#
# Goal:
#   Ship cts.ko (CipherText Stealing for CBC; one crypto_template registered). Zero new exports needed (full coverage from L50 + L51 skcipher plumbing).
...
STAGED_KO="$LKM_DIR/cts.ko"

KREL="$(uname -r)"
HOST_LIB="/lib/modules/${KREL}/kernel"
CANDIDATES=(
    "${HOST_LIB}/crypto/cts.ko"
    "${HOST_LIB}/crypto/cts.ko.xz"
)
...
if [ -z "$picked" ]; then
    echo "L52: cts.ko not present; skipping"
    exit 0
fi

echo "[test_l52_cts] picked: $picked"
...
echo "[test_l52_cts] UND ($(echo "$UND_SYMS" | wc -w)):"
for s in $UND_SYMS; do echo "  $s"; done
echo "[test_l52_cts] MISSING:"
...
(
    sleep 3
    printf 'insmod /lib/modules/6.12/cts.ko\n'
    sleep 5
    printf 'exit\n'
    sleep 1
...
if grep -E -q "PANIC|panic:" "$LOG"; then
    echo "[test_l52_cts] FAIL: kernel panic"
    exit 1
fi
...
echo "[test_l52_cts] init_OK=$INIT_OK lib_only=$LIB_ONLY fail=$INSMOD_FAIL"

if [ "$INSMOD_FAIL" -ge 1 ]; then echo "[test_l52_cts] FAIL"; exit 1; fi
if [ "$INIT_OK" -ge 1 ] || [ "$LIB_ONLY" -ge 1 ]; then
    echo "[test_l52_cts] PASS: cts.ko loaded"
    exit 0
fi
echo "[test_l52_cts] FAIL: no PASS markers"
exit 1
